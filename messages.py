...
if __name__ == "__main__":
    ...
    bookmarks = [job.split(":")[0].strip("#") for job in jobs if job.find("#") >= 0 ]
    jobs = [job.split(":")[0].strip("#") for job in jobs]
    reports = [job.split(":")[1] for job in jobs]
    job_reports = dict(zip(jobs,reports))
    jobs_build ={}
    if "lastSuccessBuild" in job_info:
        last_successful_task = job_info["lastSuccessBuild"]["url"] + "/messages/messages.json"
    ...
    for job_name in jobs:
        ...
        if job_name in jobs_build:
            ...
            if job_name in bookmarks:
                ...
                last_build = job_info["lastSuccessBuild"]["number"]
                if last_build > str(build):
                    message_json_url = job_info["lastSuccessBuild"]["url"] + "/" + job_reports[job_name] + "/messages.json"
                    response = server.jenkins_request(requests.Request('GET', message_json_url))
                    if response.status_code == "200":
                        if build_info["number"] > max_build:
                            max_build = build_info["number"]
